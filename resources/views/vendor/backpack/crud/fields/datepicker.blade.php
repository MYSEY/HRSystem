...
<div class="input-group date">
    <input
        ...
        data-init-function="bpFieldInitDatePickerElement" type="text"
        data-next-field-date="{{ $field['generatNextFieldDate'] ?? '' }}"
        data-maturity-field-date="{{ $field['generateMaturityDate'] ?? '' }}" @include(
            'crud::fields.inc.attributes'
        )>
    ...
    @push('crud_fields_scripts')
        ...
        <script>
            ...
            function bpFieldInitDatePickerElement(element) {
                ...
                $('body').on('click', '.requested-date, .first-payment-date', function() {
                    $('.total-duration-hide').hide();
                    $('.total-installment-hide').removeClass('d-none');
                });
                $('body').on('click', '.total-installment-hide', function() {
                    $('.total-installment-hide').addClass('d-none');
                    $('.total-duration-hide').show();
                });

                $fake.on('show hide change', function(e) {
                    if (e.date) {
                        ...
                        try {
                            ...
                            var step_val = $('.input-step').val();
                            var duration = $('.input-duration').val();
                            var request_date = $('.requested-date').val();
                            var first_date = $('.first-payment-date').val();
                            if ($fake.attr('data-next-field-date') != '') {
                                axios.get('{{ URL('admin/fill-first-payment-date') }}', {
                                    params: {
                                        selected_date: sqlDate,
                                        stepVal: step_val,
                                        duration: duration,
                                        get_class: "request-date",
                                        request_date: request_date
                                    }
                                }).then(function(response) {
                                    if (response.data != '') {
                                        $(".disbursed-code").val(response.data.disbursed_code);
                                        $(".investor-code").val(response.data.contract_code);
                                        $(".first-payment-date").val(response.data.first_payment_date);
                                        $("input[name='first_payment_date']").val(response.data
                                            .first_payment_date);
                                        $(".maturity-date").val(response.data.maturity_date);
                                        $("input[name='end_date']").val(response.data.maturity_date);
                                        $(".maturity-date-count").val(response.data.total_installment);
                                    }
                                });
                            } else if ($fake.attr('data-maturity-field-date') != '') {
                                axios.get('{{ URL('admin/fill-first-payment-date') }}', {
                                    params: {
                                        selected_date: first_date,
                                        stepVal: step_val,
                                        duration: duration,
                                        get_class: "first-payment",
                                        request_date: request_date
                                    }
                                }).then(function(response) {
                                    if (response.data != '') {

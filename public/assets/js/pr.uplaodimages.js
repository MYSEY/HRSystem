/* ========================================
   pr.uplaodimages v1.0.0

   Requires: pr.uplaodimages.css
             jquery 3.3.x
             bootstrap 4.3.x
   ======================================== */
...
+ function ($) {
  ...
  ImagesLoader.prototype.btnChangeClick = function (evt, obj) {
    ...
    if (filesLength > options.allowMaxFiles)
      self.ShowProgress(self.barStyle.warning, options.errormaxfiles + ": " + options.allowMaxFiles, 2000);
    ...
    (function handle(i) {
      ...
      if (i >= files.length) {
        ...
        if (self.AttachmentArray.length >= options.allowMaxFiles)
          self.$addImage.hide();
  ...
  ImagesLoader.prototype.CheckFilesCount = function () {
    ...
    return (self.AttachmentArray.length < options.allowMaxFiles);
  }
  ...
  ImagesLoader.prototype.CheckValidity = function () {
    ...
    if (self.AttachmentArray.length < options.allowMinFiles) {

      self.ShowProgress(self.barStyle.danger, options.errorminfiles + ": " + options.allowMinFiles);
      retVal = false;
    }
  ...
  ImagesLoader.prototype.init = function () {
    ...
    // Show add image
    if (this.imagesToLoad == null || this.imagesToLoad.length < this.options.allowMaxFiles)
      this.$addImage.show();
  }
  ...
  var imagesloader_fileName = "pr.uplaodimages";
